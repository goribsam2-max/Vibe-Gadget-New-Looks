  ...
  return (
    <div className="relative pt-0 pb-24 px-6 md:px-12 bg-zinc-50 dark:bg-zinc-800 max-w-[1440px] mx-auto min-h-screen font-inter">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="relative z-20 p-8 md:p-16 max-w-2xl h-full flex flex-col justify-center">
                ...
                >
                  <StatusBadge leftIcon={Sparkles} leftLabel="Featured Collection" />
                </motion.div>
                <motion.h2 
                   initial={{ opacity: 0, y: 20 }}
                   animate={{ opacity: 1, y: 0 }}
                ...
                <motion.button
                  ...
                  className="px-8 py-4 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-2xl font-semibold text-sm hover:scale-[1.02] active:scale-[0.98] transition-transform flex items-center gap-3 w-max "
                >
                  <span>Shop Now</span>
      ...
      <div className="mb-10 md:mb-16 lg:hidden">
        ...
        >
          <div
            className={`relative flex items-center bg-zinc-50 dark:bg-zinc-800 rounded-full border transition-all ${isSearchFocused ? "border-zinc-900 dark:border-zinc-100 ring-2 ring-zinc-900 dark:ring-zinc-100/20 bg-zinc-50 dark:bg-zinc-800" : "border-zinc-200 dark:border-zinc-700"}`}
          >
            ...
            />
            <button className="absolute right-4 text-zinc-900 dark:text-zinc-100 hover:text-zinc-950 dark:text-zinc-50 transition-colors">
      ...
      {/* Partner & Earn Cash (Referral Card) */}
      <div className="mb-10 w-full animate-fade-in group flex justify-center">
        {(!userData) ? (
          <ReferralCard
            badgeText="Earn commissions"
            title="Refer & Earn"
            description="Join our affiliate program and earn money for every successful referral."
            steps={[
              { icon: <Users className="h-5 w-5" />, text: "Create an account" },
              { icon: <Crown className="h-5 w-5" />, text: "Apply for affiliate status" },
              { icon: <Zap className="h-5 w-5" />, text: "Share your link & earn money" },
            ]}
            referralLink=""
            buttonText="Sign In / Sign Up"
            onButtonClick={() => navigate("/auth-selector")}
          />
        ) : userData.affiliateStatus !== 'approved' ? (
          <ReferralCard
            badgeText="Become a partner"
            title="Partner & Earn"
            description="Turn your network into net worth. Apply to be an affiliate today."
            steps={[
              { icon: <Crown className="h-5 w-5" />, text: "Apply to be a partner" },
              { icon: <Zap className="h-5 w-5" />, text: "Get your custom affiliate link" },
              { icon: <Users className="h-5 w-5" />, text: "Earn commissions on every sale" },
            ]}
            referralLink=""
            buttonText={userData.affiliateStatus === 'pending' ? "Application Pending" : "Apply for Affiliate"}
            onButtonClick={() => navigate("/affiliate")}
          />
        ) : (
          <ReferralCard
            badgeText="Active Affiliate"
            title="Refer & Earn"
            description="Share your invite link with friends and start earning."
            steps={[
              { icon: <Zap className="h-5 w-5" />, text: "Share your custom link" },
              { icon: <Users className="h-5 w-5" />, text: "Get your friends to buy" },
              { icon: <Crown className="h-5 w-5" />, text: "You earn commissions on sales" },
            ]}
            referralLink={`https://${window.location.hostname}/?ref=${userData.affiliateCode || userData.uid}`}
          />
        )}
      </div>
      ...
      <div className="animate-stagger-3 relative z-10">
        <div className="flex justify-between items-end mb-10 px-2">
          <div>
            <div className="mb-2">
              <StatusBadge leftIcon={Star} leftLabel="Our Collection" />
            </div>
        ...
        {showProof && (
          ...
          >
            ...
            <div className="flex items-center space-x-4 mb-2 pr-4">
              <div className="bg-zinc-200 dark:bg-zinc-700 text-zinc-800 dark:text-zinc-200 w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-inner">
            ...
            <div className="border-t border-zinc-100 dark:border-zinc-800 pt-2 mt-1">
              <label className="flex items-center space-x-2 cursor-pointer group">
                <input
                  type="checkbox"
                  className="w-3 h-3 text-zinc-800 dark:text-zinc-200 rounded focus:ring-zinc-900 dark:ring-zinc-100 border-zinc-300"
                  onChange={(e) => {
                    if (e.target.checked) {
